...
def load_data_names(path):
    ...
    #for seq in seqs:
    for seq in seqs:
       # file_path = sorted(glob.glob(join(path,file)))
        f = open(seq)
...
def load_batch_from_names_random(names, path, batch_size, img_cols, img_rows, img_ch):
    ...
    while b < batch_size:
        ...
        br_y = tl_y + h
        face_grid[0, tl_y:br_y, tl_x:br_x] = 1
        # print('face grid values', face_grid)
        ...
        # save images (for debug)
        if save_img:
            cv2.imwrite(r"D\face.png", face)
            cv2.imwrite(r"D:\right.png", right_eye)
            cv2.imwrite(r"D:\left.png", left_eye)
...
if __name__ == "__main__":
    ...
    # debug
    seq_list = load_data_names(r"C:\Users\Aliab\PycharmProjects\data_small\test")
    ...
    test_batch = load_batch_from_names_random(seq_list, dataset_path, batch_size, img_cols, img_rows, img_ch)
